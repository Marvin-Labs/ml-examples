...
In this example we are using distilbert-base-uncased, which is a simple yet effective embedding model, but you can
use any model which suits your application.
...
# Collect metadata mapping event indices to the index of their origin document
...
small_events = [(event.get('Predicate', None), event.get('Agent', None), event.get('Patient', None),
                 event.get('Goal', None), event.get('Recipient', None)) for event in events]
