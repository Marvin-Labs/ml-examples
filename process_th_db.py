...
for i, doc_rep in enumerate(db):
    # If for any reason there is a formatting issue that causes the evaluation to fail,
    # just drop it. It's not worth worrying about.
    try:
        doc_rep = eval(doc_rep)
    except:
        doc_rep = []
    # If the doc is only one event long it would be represetend as a dict
    # rather than a list of dicts. So we create a 1-element list out of it
    # to fit the format.
...
# We define it here rather than above because it uses the clusters we just found.
...
# Collapse each NP into its cluster ID to compress the dataset.
    # I know it's messy, but it reflects the fact that ChatGPT's output has artifacts in it
collapsed_db = []
non_collapsed_db = []
